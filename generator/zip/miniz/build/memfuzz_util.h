...
#include <stdint.h>
#include <time.h>
extern jmp_buf memfuzz_jmp_buf; //use it to jump to printer function
...
char* gen_random_str(int length); // Caller takes the ownership
/*String*/
...
// Create a new array and copy the content in arr. Used for array shuffle
void* gen_array_copy(void* arr, size_t n, size_t size); // Caller takes the ownership
// Increase the length of an array and return a new one with new_size
void* gen_array_increase(void* arr, size_t n0, size_t size, size_t n1); // Caller takes the ownership
