...
#define PRINT_S 
//#define PRINT_S std::cout<<s<<std::endl;
/*Modification End*/
...
//------------------------------------------------------------------------
/*Modification Begin*/
std::vector<Object*> XRef::get_all_objs()
{
    if(this->objs.size() > 0)
    {
        return this->objs;
    ...
    int i;
    XRefEntry e;
    this->objs.resize(this->last + 1, NULL);
    this->gens.resize(this->last + 1, 0);
    for(i = 0; i <= this->last; i++)
    {
        ...
        this->objs[i] = new Object();
        this->gens[i] = e.gen;
        this->fetch(i, e.gen, this->objs[i]);
    }
    return objs;
}

//transform all objects in this->objs to a pdf. This function construct the xref and do not use the existing one.
std::string XRef::to_string()
{
    int i;
    unsigned startxref;
    std::string total;
    ...
    if(this->objs.size() != this->last + 1 || this->objs.size() != this->gens.size())
    {
        std::cout<<"Warning: XRef::to_string() should be invoked after get_all_objs()\n";
        exit(0);
    }

    //construct the header
    total = "%PDF-1.7\r\n%%EOF\r\n";
    s_xref = "xref\r\n0 " + std::to_string(this->last + 1) + "\r\n";
    //construct the body and xref
    for(i = 0; i <= this->last; i++)
    {
        if(i == 0)
        {
            s_xref += "0000000000 65535 f\r\n";
            continue;
        }
        //an emptry entry
        if(objs[i] == NULL)
        {
            s_xref += "0000000000 00000 f\r\n";
            continue;
        }
        
        ss.str(std::string());
        ss<<std::setw(10)<<std::setfill('0')<<total.length();
        s = ss.str();
        s_xref += s;
        total += std::to_string(i) + " 0 obj\r\n" + objs[i]->to_string() + "\r\nendobj\r\n\r\n";      
        
        ss.str(std::string());
        ss<<std::setw(5)<<std::setfill('0')<<this->gens[i];
        s = ss.str();
        s_xref += " " + s + " n\r\n";
    }
    startxref = total.length();
    total += s_xref;
    total += "trailer\r\n";
    //obj = this->trailerDict.dup();
    
    total += this->trailerDict.trailer_to_string();
...
int XRef::mutate(int ref, int *status)
{
    if(ref <= 0 || ref > this->last || this->objs.size() != this->last + 1 || this->objs[ref] == NULL)
        return 0;
    //a dup of cur obj (invoke dup())
    Object *save;
    //a copy of cur obj (=)
    Object val;
    //some tmp pointer, tmp is the object under mutation
    Object *tmp,  *tmp1, *tmp2;
    //the string of the generated pdf
    std::string s;
    //iterator
    int i, j;
    //length of array, dict or stream
    int length;
    //the total mutation cnt
    int cnt = 0;
    //the name of a key
    char* name;
    //the dict used in stream
    Dict* d;
    
    std::vector<Object> vec;
    //some object covering different types and values
    std::vector<Object> type_checks = objPool.getChecks();
    //the stack of object, preorder mutate
    std::stack<Object*> stk;
    //ctl for fuzzing
    int ctl;
    
    stk.push(this->objs[ref]);
    while(!stk.empty())
    {
      //break in case of object too large
      if(cnt > 50000)
          break;
      tmp = stk.top();
      stk.pop();
      ...
      /*Stage 1: Type Check and Value Check*/
      val = *tmp; //save the original value
      //substitute the current object
      for(i = 0; i < type_checks.size(); i++)
      {
          *tmp = type_checks[i];
          s = this->to_string();
          cnt++;
      ...
      //ref: add an object to match the ref
      this->last++;
      tmp->type = objRef;
      tmp->ref.num = this->last;
      tmp->ref.gen = 0;
      this->gens.push_back(0);
      for(i = 0; i < type_checks.size(); i++)
      {
          this->objs.push_back(&type_checks[i]);
          s = this->to_string();
          cnt++;
#ifdef DEBUG_MODE
          PRINT_S
          ...
          {
              this->objs.erase(this->objs.begin() + this->last);
              this->gens.erase(this->gens.begin() + this->last);
              this->last--;
              goto early;
          }
          else
      ...
      *tmp = val;
      ...
      /*Stage 2: Type Specific Mutation*/
      //inner type
      switch (tmp->type)
      {
      case objBool:
      ...
      //complex type
      //array: delete, add element
      case objArray:
        length = tmp->arrayGetLength();
        //add a new elem
        tmp1 = new Object();
        tmp1->initInt(0);

        tmp->array->insert(0, tmp1);
        s = this->to_string();
        cnt++;
#ifdef DEBUG_MODE
          PRINT_S
        ...
        tmp1 = tmp->array->getDup(length - 1);
        tmp->array->remove(length - 1);
        s = this->to_string();
        cnt++;
#ifdef DEBUG_MODE
        PRINT_S
        ...
        {
            tmp->array->insert(length-1, tmp1);
            delete tmp1;
            goto early;
        }
        else
        ...
        {
          tmp1 = tmp->array->getPointer(i);
          stk.push(tmp1);
        }
        break;

      case objDict:
        length = tmp->dictGetLength();
        vec = objPool.getKeys();
        //keys, type_checks
        for(i = 0; i < vec.size(); i++)
        {
          //basic type
          for(j = 0; j < type_checks.size(); j++)
          {
            tmp1 = type_checks[j].dup();
            tmp->dict->add(strdup(vec[i].getName()), tmp1);
            delete tmp1;
            s = this->to_string();
            cnt++;
#ifdef DEBUG_MODE
          ...
          tmp1 = new Object();
          tmp1->type = objRef;
          tmp1->ref.num = this->last;
          tmp1->ref.gen = 0;
          tmp->dict->add(strdup(vec[i].getName()), tmp1);
          //checks
          for(j = 0; j < type_checks.size(); j++)
          {
            this->objs.push_back(&type_checks[j]);
            s = this->to_string();
            cnt++;
#ifdef DEBUG_MODE
            PRINT_S
          ...
          //stream
          tmp2 = objPool.random_stream();
          this->objs.push_back(tmp2);
          s = this->to_string();
          cnt++;
#ifdef DEBUG_MODE
          PRINT_S
        ...
        for(i = length - 1; i >= 0; i--)
        {
          stk.push(tmp->dict->get(i));
        }
        break;

      case objStream:
        if(tmp->stream == NULL || tmp->stream->getDict() == NULL)
            break;
        d = tmp->stream->getDict();
        length = d->getLength();
        vec = objPool.getKeys();
        //keys, type_checks
        for(i = 0; i < vec.size(); i++)
        {
          ...
          for(j = 0; j < type_checks.size(); j++)
          {
            d = tmp->stream->getDict();
            tmp1 = type_checks[j].dup();
            d->add(strdup(vec[i].getName()), tmp1);
            delete tmp1;
            s = this->to_string();
            cnt++;
#ifdef DEBUG_MODE
          ...
          //the dictionary in stream is not likely to include another stream
        }
      ...
      case objRef:
        //change to all possible ref
        for(i = 1; i < this->objs.size(); i++)
        {
            if(this->objs[i] == NULL)
                continue;
            tmp->ref.num = i;
            tmp->ref.gen = this->gens[i];
            s = this->to_string();
            cnt++;
#ifdef DEBUG_MODE
            PRINT_S
        ...
        tmp->ref = save->ref;
        //Is it necessaryto change the ref to a new object and create a obj to match it?
      ...
      /*Stage 3: ObjectPool Random mutation*/
      /*End Stage 3*/
