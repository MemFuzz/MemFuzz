...
def query(struct_name, target):
    ...
    cmd = path + " -M -I -q -a --class_name=" + struct_name + " " + target + " > /tmp/one_class"
    msg = os.system(cmd)
    assert(msg == 0)
    enum = ""
    if os.path.exists("./memfuzz_enum.h"):
...
def next_token(buf):
    ...
    for i in range(len(buf)):
        if template_cnt > 0:
            ...
            # special check to get the token
            if buf[i] == '_' or buf[i].isalpha() or buf[i].isdigit() or buf[i] == '$' or buf[i] == ':':
                token += buf[i]
                index += 1
            elif buf[i] == '<':
                template_cnt += 1
...
class Generator:
    ...
    def gen_struct_member_mutate(self, class_member, prefix):
        ...
        if class_member.is_base():
            ...
            # Base type, do interesting, add/minus/shift/random
                ...
                #interesting
                mutate_type = Values.get_values(base_type)
                code = "assign_" + mutate_type + "(&(ptr->" + class_member.name + "), level+1);\n"
                #...
            ...
            # structre* or structure[]
                ...
                # pointer of structure, usually it just point to one object. It can also be dynamic array, we may check it.
                    ...
                    if struct_type[0] != None:
                        code = "if(ptr->" +class_member.name + ") mutate_" + struct_type[0] + "(ptr->" + class_member.name + ", level+1);\n"
                    else:
                        code = "//ptr->" + class_member.name + " = ...(" + str(class_member) + ")\n"
                # array of structure
                else:
                    ...
                    if struct_type[0] != None:
                        ...
                        # we sample all element here
                            if dimensions[0] < self.sample_times:
                                code = "for(int i = 0; i < " + str(dimensions[0]) + "; i++)\n    {\n        mutate_" + struct_type[0] + "(&(ptr->" + class_member.name + ")[i], level+1);\n    }\n"
                        # choose samples_times elements to mutate
                            else:
                                code = "for(int i = 0; i < SAMPLE_TIMES; i++)\n    {\n        mutate_" + struct_type[0] + "(&(ptr->" + class_member.name + ")[random()%" + str(dimensions[0]) + "], level+1);\n    }\n"
                    # anonymous structure
                    else:
                        code = "//ptr->" + class_member.name + " = ...(" + str(class_member) + ")\n"
            # structure
                ...
                # named structure
                if struct_type[0] != None:
                    code = "mutate_" + struct_type[0] + "(&(ptr->" + class_member.name + "), level+1);\n"
                # anonymous structure
                else:
                    code = "//ptr->" + class_member.name + " = ...(" + str(class_member) + ")\n"
    ...
    def gen_struct_mutate(self, class_name):
        ...
        prototype = "void mutate_" + struct.name + "(void* ptr1, int level)"
...
class DAG:
    ...
    def extract_struct(self, target):
        ...
        cmd = path + " -a -A --show_private_classes -M -I " + target + " > /tmp/class.h"
        msg = os.system(cmd)
        if msg != 0:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    target = sys.argv[1]
    gen_classinfo = "./gen_classinfo -a -A --show_private_classes -M" # third/pahole/src/pahole
