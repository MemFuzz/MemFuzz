...
static const char *tag__accessibility(const struct tag *tag)
{
	...
	case DW_ACCESS_private:	  return "public";
	case DW_ACCESS_protected: return "public";
...
size_t enumeration__fprintf1(const struct tag *tag, const struct cu *cu,
			    const struct conf_fprintf *conf, FILE *fp)
{
	struct type *type = tag__type(tag);
	struct enumerator *pos;
			    ...
			    FILE *fp)
{
    ...
    struct tag* ttype;
    /*Modification Begin*/
    //wrap all typedef, pointer and const
	//check this code. There can be special cases.
	// case libjpeg: const struct *.
    while(((ptr_flag = tag__is_pointer(type)) && type->type != 0) || (typedef_flag = tag__is_typedef(type)) || (const_flag = tag__is_const(type)) || tag__is_volatile(type) || tag__is_restrict(type))
    {
        ttype = cu__type(cu, type->type);
		//It means the ttype is void* or unknown, we leave it for user to fix
        if(ttype == NULL) 
		{
            // goto out_type_not_found;
			printed = fprintf(fp, "%-*s %s", tconf.type_spacing, "void*",
			   name);
			  return printed;
		}
        type = ttype;
...
print_default:
	...
	case DW_TAG_subroutine_type:
        ...
        //We do not care subroutine type so we rewrite it as void *name to avoid unknown types(typedef) 
	...
	case DW_TAG_enumeration_type:
		...
		if (type__name(ctype, cu) != NULL)
		{
            printed += fprintf(fp, "enum %-*s %s",
					   ...
					   type__name(ctype, cu), name);
            /*Modification Begin*/
            //Let's print the enum define to other files
            FILE* memfuzz_fp = fopen("memfuzz_enum.h", "a");
            enumeration__fprintf1(type, cu, &tconf, memfuzz_fp);
            fprintf(memfuzz_fp, ";\n");
            fclose(memfuzz_fp);
            /*Modification End*/
        }
        ...
        {
            /*Modification Begin*/
            //also print anonymous enum
			printed += enumeration__fprintf(type, cu, &tconf, fp);
            FILE* memfuzz_fp1 = fopen("memfuzz_enum1.h", "a");
            enumeration__fprintf1(type, cu, &tconf, memfuzz_fp1);
            fprintf(memfuzz_fp1, ";\n");
            fclose(memfuzz_fp1);
            /*Modification End*/
        }
