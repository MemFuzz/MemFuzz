...
static size_t type__fprintf1(struct tag *type, const struct cu *cu,
                const char *name, const struct conf_fprintf *conf,
                FILE *fp)
{
    int typedef_flag = 0;
    int ptr_flag = 0;
    if(name)
        printf("MemberName(%s) ", name);
    //we make the anonymous null+num
    else
        printf("MemberName(null) ");
    int nr_indirections = 0;//the level of pointer
    struct type* ctype;
    if(type == NULL)
    {
        printf("DW_TAG_unknown ");
        return 0;
    }
    //print pointer and typedef
    while((ptr_flag = tag__is_pointer(type) && type->type != 0) || (typedef_flag = tag__is_typedef(type)))
    {
...
next_type1:
    ...
    case DW_TAG_pointer_type:
        if(type->type == 0)
        {
            printf("DW_TAG_pointer_type DW_TAG_base_type(void) ");
            break;
        }
        else
        {
        ...
        //We do not care about function, ignore it
        if(ptype->tag == DW_TAG_subroutine_type)
        {
            break;
        }
        // I do not think it can reach here, but I put the code here anyway
        // An inner struct so we can not find its name
        if((tag__is_struct(ptype) || tag__is_union(ptype) || tag__is_enumeration(ptype)) 
                && type__name(tag__type(ptype), cu) == NULL)
        {
            
        }
            
        break;
    // We do not care about functions
    case DW_TAG_subroutine_type:
    ...
    case DW_TAG_const_type:
        ...
        */
        break;
    // Print dimension as well as length
    case DW_TAG_array_type:
        ...
        int i;
        printf("%d ", at->dimensions);
        for(i = 0; i < at->dimensions; i++)
        {
            printf("%d", at->nr_entries[i]);
            if(i != at->dimensions - 1)
                printf(" ");
        }
        printf(") ");
        struct tag* array_tag = cu__type(cu, type->type);
        if(array_tag != NULL)
        {
            type = array_tag;
            goto next_type1;
    ...
    case DW_TAG_structure_type:
        ...
        else
        {
            ...
            struct type* class_type = &class->type;
            struct tag* tag_pos;
            struct class_member* pos;
            type__for_each_tag(class_type, tag_pos)
            {
                pos = tag__class_member(tag_pos);
                struct tag* pos_type = cu__type(cu, pos->tag.type);
                //print the class member type
                if(pos_type == NULL)
                {
                    continue;
                }
                const char* cm_name = class_member__name(pos, cu);
                //if(pos->is_static)
                //{
                    //printf("static ");
                //}
                ...
                printf("[ ");
                type__fprintf1(pos_type, cu, cm_name, conf, fp);
				if(pos->bitfield_size != 0)
				{
					printf("DW_TAG_bitfield(%u) ", pos->bitfield_size);
				}
                printf(" ] ");
            }
            printf("} ");
    ...
    case DW_TAG_enumeration_type:
        ...
        struct enumerator *pos;
        int num = 0;
        type__for_each_enumerator(ctype, pos)
            num++;
        printf("(%d) ", num);
        /*//We just ignore the enum type name as it does not matter
        if(type__name(ctype, cu) != NULL)
        {
            printf("%s ", type__name(ctype, cu));
        }
    ...
    default:
        ...
        case DW_TAG_base_type:
        {
            ...
            //find the name of the base type
            const struct base_type *bt = tag__base_type(type);
            const char* bt_name = "void";
            bt_name = cu__string(cu, bt->name);
            printf("(%s) ", bt_name);
            break;
        }
        default:
        {
            printf("DW_TAG_unknown(%s)", dwarf_tag_name(type->tag));
        }
        }
    }
    return 0;
...
/*This is the entry for class printing*/
		    ...
		    const struct conf_fprintf *conf, FILE *fp)
{
	...
	case DW_TAG_structure_type:
        //printed += __class__fprintf(tag__class(tag), cu, pconf, fp); /*Original code*/
        /*Modification Begin*/
        class = tag__class(tag);
        type = &class->type;
        class_name = type__name(type, cu);
        if (tag->tag == DW_TAG_class_type)
            // printf("ClassName(%s)\n", class_name);
			// change the access of class
            printf("StructName(%s)\n", class_name);
        else
            printf("StructName(%s)\n", class_name);
        ...
        type__for_each_tag(type, tag_pos){
			...
			// a static member but we do not want to mutate it so ignore it
			if(pos->is_static)
            {
                // printf(" DW_TAG_static ");
				continue;
            }
            const char* cm_name = class_member__name(pos, cu);
			
            //print the  class member
            printf("[ ");
            type__fprintf1(pos_type, cu, cm_name, &cconf, fp);
			// it is a bitfield member
			if(pos->bitfield_size != 0)
			{
				printf(" DW_TAG_bitfield(%u)", pos->bitfield_size);
			}
