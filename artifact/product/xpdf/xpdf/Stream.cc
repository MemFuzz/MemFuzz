...
FileStream::FileStream(FILE *fA, GFileOffset startA, GBool limitedA,
  ...
  bufPos = start;
  /*Modification Begin*/
  //We should save the content in the stream at the very beginning in case the file pointer is closed later
  len = 0;
  content = (char*)malloc(lengthA + 1);
  if(content != NULL)
  {
    len = this->getBlock(content, lengthA);
    content[len] = 0;
    this->reset();
  }
  /*Modification End*/
...
/*Modification Begin*/
//Note that the file pointer is not duplicated here. It is okay because the destructor does not invoke fclose
Stream *FileStream::dup()
{
    Object *dictA;
    dictA = dict.dup();
    Stream* tmp = new DupStream(dictA, len, content);
    delete dictA;
    return tmp;
}
/*Modification End*/
...
/*Modification Begin*/
DupStream::DupStream(Object* dictA, int lenA, char* contentA):BaseStream(dictA)
{
    this->len = lenA;
    if(this->len == 0)
        this->content = NULL;
    else
        this->content = strdup(contentA);
}
...
char* DupStream::get_content(int *lenA)
{
    *lenA = this->len;
    return this->content;
}

Stream* DupStream::dup()
{
    Object *dictA = this->dict.dup();
    Stream* tmp = new DupStream(dictA, this->len, this->content);
    delete dictA;
    return tmp;
}
/*Modification End*/
