...
/*Global Variables*/
unsigned char* a_file; // shared memory pointer for communicating with AFL
...
static int timeout = 2000;//Set timer for safe_print
/*Global Variables*/
...
//Not in use, I think use a shared memory for probing is better
...
static void setup_stats_shm(void)
{
    ...
    memset(memfuzz_stats, sizeof(MemStats), 0);
}
...
void safe_print(const char* outfile)
{
    ...
    else if(pid > 0)
    {
        ...
        fclose(fp);
        //Send file with 0 length means mutation is done, so we should not send this file
        if(length == 0)
        {
            return ;
        }
