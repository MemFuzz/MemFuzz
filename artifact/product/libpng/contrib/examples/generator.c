...
int main(int argc, const char **argv)
{
   ...
   /*Modification Begin*/
   argc = 3;
   argv[1] = MEMFUZZ_INFILE;
   argv[2] = MEMFUZZ_OUTFILE;
   memfuzz_init();
   /*Modification End*/
   if (argc == 3)
   {
      png_image *image;
      image = (png_imagep)malloc(sizeof(png_image));
      ...
      if (png_image_begin_read_from_file(image, argv[1]))
      {
         ...
         if (buffer != NULL)
         {
               ...
               0/*row_stride*/, NULL/*colormap for PNG_FORMAT_FLAG_COLORMAP */))
            {
               ...
               {
                  // mutate_int8_t_array(buffer, 0, PNG_IMAGE_SIZE(*image));
                  exit(0);
               }
               /*Modification End*/
