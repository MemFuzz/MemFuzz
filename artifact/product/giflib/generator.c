...
int main(int argc, char **argv)
{
	...
	int fd0 = open(MEMFUZZ_INFILE, O_RDONLY);
    /* read in a GIF */
    if ((GifFileIn = DGifOpenFileHandle(fd0, &ErrorCode)) == NULL) {
	...
	remove(MEMFUZZ_OUTFILE);
	int fd1 = open(MEMFUZZ_OUTFILE,  O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR);
	if ((GifFileOut = EGifOpenFileHandle(fd1, &ErrorCode)) == NULL) {
	PrintGifError(ErrorCode);
	exit(EXIT_FAILURE);
    }
