...
int main(int argc, char **argv)
{
    ...
    if (NumFiles == 1) {
	if ((GifFileIn = DGifOpenFileName(MEMFUZZ_INFILE, &ErrorCode)) == NULL) {
    ...
    //Put mutation before opening out fd to avoid race condition
    ...
    //open MEMFUZZ_OUTFILE instead
	int fd = open(MEMFUZZ_OUTFILE,  O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR);
	if ((GifFileOut = EGifOpenFileHandle(fd, &ErrorCode)) == NULL) {
	PrintGifError(ErrorCode);
	exit(EXIT_FAILURE);
    }
